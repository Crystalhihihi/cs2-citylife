...
namespace CityLife.Content
{
    ...
    public static class ChatterSpec
    {
        ...
        /// <summary>口吻结构签词表（12 词，#72 转正+扩编；甩话头归对话结构签不进独白，别往里加）。</summary>
        ...
        // 六性格 × 12 口吻 权重表（如何调：数值=相对权重，0=不选；行序与 Personalities、列序与 Voices 对齐）
        // 碎嘴→碎嘴短句/复读/反问连发/拟声高权；闷葫芦→省略号压倒性高权+少量短句；
        // 毒舌→反问/反问连发/自嘲高权；慢性子→嘛咯嘞/重复半句/省略号高权；
        // 杠精→反问/反问连发压倒高权；热心肠→啊呗呢/拟声/叹词高权
        private static readonly int[][] k_VoiceWeights =
        {
            new[] { 1, 1, 2, 1, 4, 3, 1, 2, 4, 4, 3, 1 }, // 碎嘴
            new[] { 1, 0, 0, 0, 2, 1, 5, 0, 1, 0, 0, 1 }, // 闷葫芦
            new[] { 1, 0, 3, 1, 1, 1, 1, 0, 0, 3, 0, 3 }, // 毒舌
            new[] { 1, 3, 0, 1, 1, 3, 3, 0, 1, 0, 1, 1 }, // 慢性子
            new[] { 1, 0, 4, 1, 1, 1, 1, 0, 0, 4, 1, 2 }, // 杠精
            new[] { 4, 2, 1, 1, 1, 1, 1, 2, 1, 0, 3, 1 }, // 热心肠
        };
            ...
            string[] moods)
        {
            ...
            for (var step = 0; step < total; step++)
            {
                ...
                // 口吻按性格权重表加权取（同一（写,情）组合下口吻也随签序变——组合空间 3×10×12=360 ≫ 炉卡数）
                var v = PickVoice(weights, salt + (uint)step + (uint)cardIdx);
                if (used.Add((s, m, v)))
                    return (s, m, v);
            }
        ...
        /// <summary>性格权重表内加权取签（确定性：roll 锚定盐值）。</summary>
