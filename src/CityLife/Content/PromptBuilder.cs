...
namespace CityLife.Content
{
    ...
    public static class PromptBuilder
    {
        ...
        public static string BuildChatterHead()
        {
            ...
            sb.Append("【配额】萌宠题材（猫/狗/宠物）全炉至多 1 条；且萌宠不当风景看——要么进对话要么带情节（叼了啥/拆了啥/要闯祸了），光蹲那被观察的一律判废。\n"); // §12 #72 二批萌宠双轨·prompt 轨
            ...
            sb.Append("【对话】卡文带\"｜对：\"的是双人卡（\"｜对：\"后是另一人的处境）：两人对话一句起一句应，像街坊擦肩接话，a/b 各 ≤12 字，可聊两人的处境/旁边的东西/题里的话头；台词禁喊名字禁自称（显示名系统自动加）。对话别对称问答：允许信息不齐（忘了/猜区间\"9点还是10点来着\"/反问回去\"买啥了？\"）；允许（点点头）这种没字的拍、（突然回头）这种收束拍——但禁\"笑了笑\"\"指着远处\"式神态描写；允许下判断泼冷水（\"你这包拿不到的\"）；允许规则边缘的小算盘（\"反正没人查\"——别个个都那么乖）；结尾允许甩个新话头不闭环（\"哎周末钓鱼去啊！\"）。\n"); // §12 #72 二批对话结构签
                                                ...
                                                IReadOnlyList<int>? pairCardNos = null)
        {
            ...
            if (hasVoice)
                sb.Append("，\"｜口：\"是这人的说话口气——这张卡里挑一条带上这个口气就够，其余自由发挥"); // §12 #72 二批"一卡一条"（把戏密度÷3：签管一张卡里最像的那一条，不管每条）
            if (hasMicro)
                sb.Append("，\"｜微：\"是这人手头正在干的事（顺嘴一提，别当题）"); // §12 #72 二批⑥
            if (hasScene)
            ...
            if (hasScene)
                sb.Append("【场景】带\"｜景：\"的卡：这人此刻就在那个场所里，话要从那个场所里长出来——在那办事、等人、干活、陪人，说的就是那地方的事。\"｜话核：\"别广播——写从关系里长出来的话：唠叨/催促/埋怨/搭话，话核当背景不当题；顺着它自然聊，别逐字复读、别当成必答清单。没\"｜景：\"的卡自由发挥。\n"); // §12 #71 措辞优化（2026-09-20 实验定案）+ #72 二批"关系姿势"（校门口=家长唠叨喝水别跑，不是聊"作业"这个词）
            if (hasSpec)
        ...
        public static string BuildTheaterStockHead()
        {
            ...
            sb.Append("【结构】对话要有真结构，别对称问答背书：信息允许不齐——忘了（\"9点还是10点来着\"）、猜区间、反问回去（\"买啥了？\"），别一问一答全对上；非语言回合可以有——（点点头）独占一拍没字；可以下判断泼冷水（\"你这包拿不到的\"\"一看就是电瓶在充\"）；允许规则边缘的小算盘（\"反正没人查\"\"偷偷出去一会\"——别个个都那么乖）；（说了一堆话）这种省略拍可以有，跳过别人说话的叙事；愿望/决心不许第一拍就出现（\"能拿上就好了\"是被剧情挣出来的，别开场就许愿）；开场可以打发式（\"嗨甭说了\"），接话可以回咬上文的线头，结尾允许甩个新话头不闭环（\"（突然回头）哎周末钓鱼去啊！\"）。\n"); // §12 #72 二批对话结构签（剧场 stock 头）
